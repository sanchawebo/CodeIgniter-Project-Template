<?php
/**
 * @var CodeIgniter\View\View   $this
 * @var CodeIgniter\Pager\Pager $pager
 */
?>
<div class="col" id="user-list">
  <form
    action="<?= site_url('/admin/users/delete-batch') ?>"
    method="post">
    <?= csrf_field() ?>
    <table class="m-table mt-5">
      <?= $this->include('Mpr\UserMgmt\Views\_table_head') ?>
      <tbody>
        <?php if (isset($users) && count($users)) : ?>
        <?php foreach ($users as $user) : ?>
        <tr>
          <?php if (auth()->user()->can('users.delete')): ?>
          <td>
            <div class="a-checkbox d-table">
              <input
                type="checkbox"
                name="selects[<?= $user->id ?>]"
                id="checkbox-<?= $user->id ?>"
                class="d-none"
              >
              <label for="checkbox-<?= $user->id ?>">&nbsp;</label>
            </div>
          </td>
          <?php endif ?>
          <?= view('Mpr\UserMgmt\Views\_row_info', ['user' => $user], ['saveData' => false]) ?>
        </tr>
        <?php endforeach ?>
        <?php endif ?>
      </tbody>
    </table>

    <?php if (auth()->user()->can('users.delete')) : ?>
    <button type="submit" value="Delete Selected" class="a-button a-button--secondary mt-2">
      <i class="a-icon a-button__icon boschicon-bosch-ic-alert-warning"></i>
      <span class="a-button__label"><?= lang('Users.deleteSelected') ?></span>
    </button>
    <?php endif ?>

    <div class="mt-5 hstack justify-content-center">
      <?= $pager->links('default', 'custom_full') ?>
    </div>
  </form>
</div>